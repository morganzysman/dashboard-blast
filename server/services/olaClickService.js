...
/**
 * Fetch raw OlaClick orders for an account & date range. Returns the array of
 * order objects exactly as OlaClick returned them. Used by the daily-gain
 * rollup and any caller that wants the full list.
 *
...
export async function fetchGeneralIndicators(account, queryParams = {}) {
  ...
  try {
    ...
    console.log(`📦 Fetched ${allOrders.length} order rows (meta.total=${meta.total ?? 'n'}, pages=${page})`);
