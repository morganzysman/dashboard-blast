<template>
  <div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">⚠️ My Warnings</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">
          View warnings issued to your account and acknowledge them when reviewed.
        </p>
      </div>
      ...
      <div class="card">
        <div class="card-body">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <p class="text-sm text-gray-600 dark:text-gray-400">Total Warnings</p>
      ...
      <div class="card">
        <div class="card-body">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <p class="text-sm text-gray-600 dark:text-gray-400">Pending Acknowledgment</p>
      ...
      <div class="card">
        <div class="card-body">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <p class="text-sm text-gray-600 dark:text-gray-400">Acknowledged</p>
    ...
    <div class="card">
      <div class="card-header">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">All Warnings</h2>
      </div>
      <div class="card-body">
        <!-- Loading State -->
        ...
        <div v-else-if="warnings.length === 0" class="text-center py-8">
          ...
          <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Great Work! No Warnings</h3>
          <p class="text-gray-600 dark:text-gray-400">Keep up the excellent performance and professional behavior.</p>
        </div>
        ...
        <div v-else class="space-y-4">
          <div v-for="warning in warnings" :key="warning.id" 
            ...
            <div v-if="warning.description" class="bg-white dark:bg-gray-900 rounded-lg p-3 mb-3">
              <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100 mb-1">Details:</h4>
            ...
            <div class="text-xs text-gray-500 dark:text-gray-400 space-y-1">
              <div class="flex justify-between">
                <span><strong>Issued:</strong> {{ formatDate(warning.issued_at) }}</span>
                <span><strong>By:</strong> {{ warning.issued_by_name }}</span>
              </div>
              <div v-if="warning.acknowledged_at">
                <span><strong>Acknowledged:</strong> {{ formatDate(warning.acknowledged_at) }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="card mt-6">
      <div class="card-header">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">📋 Warning Categories</h2>
      </div>
      <div class="card-body">
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Understanding warning categories helps maintain workplace standards and professional behavior.
        </p>
          ...
          <div v-for="(category, key) in warningCategories" :key="key" class="border rounded-lg p-4 dark:border-gray-700">
            ...
            <ul class="text-sm text-gray-600 dark:text-gray-400 space-y-1">
              ...
              <li v-if="category.motives.length > 3" class="text-xs italic">
                ...and {{ category.motives.length - 3 }} more
              </li>
            </ul>
          </div>
        </div>
  ...
  try {
    ...
    if (response.success) {
      ...
      window.showNotification?.({
        ...
        message: 'Thank you for acknowledging this warning.'
      })
    }
  } catch (error) {
    ...
    window.showNotification?.({
      type: 'error',
      title: 'Error',
      message: error.response?.data?.error || 'Failed to acknowledge warning'
    })
  } finally {
